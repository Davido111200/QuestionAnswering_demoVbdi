...
import collections
import json
from evaluate_squad import get_raw_scores, make_eval_dict, squad_evaluate
...
def get_score1(args):
    ...
    for (key, entry_map) in all_nbest.items():
        ...
        #         best_text = text
        #         break
        output_predictions[key] = sorted_texts[0]
    ...
    eval_rs = squad_evaluate(examples, output_predictions, output_scores, best_th)
    return eval_rs
...
def main():
    ...
    eval_rs = get_score1(args)
    print(json.dumps(eval_rs, indent= 4))

    # examples = get_examples(args.predict_file, is_training= False)
    # f1, em = get_raw_scores(examples, preds)
    # print(make_eval_dict(f1, em))
